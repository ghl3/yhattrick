...
def _career(boxes: dict[int, pd.DataFrame]) -> pd.DataFrame:
    ...
    g = g.merge(
        pd.DataFrame({"team": latest, "teams": teams}).reset_index(), on="player_id", how="left"
    )
    return g
...
def _shottypes(seasons) -> dict[int, list]:
    ...
    frames = [
        pd.read_parquet(C.PROCESSED / "goalie_shottype" / f"{s}.parquet")
        for s in seasons
        if (C.PROCESSED / "goalie_shottype" / f"{s}.parquet").exists()
    ]
    if not frames:
        return {}
    df = (
        ...
        .agg(sa=("sa", "sum"), xga=("xga", "sum"), ga=("ga", "sum"), sog=("sog", "sum"))
        .reset_index()
    )
        ...
        out[int(r.player_id)].append(
            {
                ...
                "sa": sog,  # shots on goal of this type
...
def _gamelogs(seasons) -> dict[int, list]:
    frames = [
        pd.read_parquet(C.PROCESSED / "goalie_gamelog" / f"{s}.parquet")
        for s in seasons
        ...
        out[int(r.player_id)].append(
            {
                ...
                "home": bool(r.home),
                "decision": (
                    None if (isinstance(r.decision, float) and pd.isna(r.decision)) else r.decision
                ),
                "toi_s": int(r.toi_s),
                "sog_against": int(r.sog_against),
                "saves": int(r.saves),
                "ga": int(r.ga),
                "xga": round(float(r.xga), 2),
                "gsax": round(float(r.gsax), 2),
                "started": bool(r.started),
                "shutout": bool(r.shutout),
                "qs": bool(r.qs),
                "rbs": bool(r.rbs),
            }
        )
...
def main() -> None:
    ...
    for r in g.itertuples():
        ...
        metric = {
            m: {
                "v": _f(getattr(r, m), 4 if m in ("sv_pct", "hd_sv_pct") else 2),
        ...
        index.append(
            {
                ...
                "sv_pct": _f(r.sv_pct, 4),
                "gaa": _f(r.gaa, 2),
                "xga": _f(r.xga, 1),
                "gsax": _f(r.xga - r.ga, 1),
                "gsax_per100": _f(r.gsax_per100, 2),
                "gsax_per100_se": _f(r.gsax_per100_se, 2),
                "gsax60": _f(r.gsax60, 2),
                "hd_sv_pct": _f(r.hd_sv_pct, 4),
                "qs": int(r.qs),
                "rbs": int(r.rbs),
                "qs_pct": _f(r.qs_pct, 3),
                **{f"{m}_pct": _f(getattr(r, f"{m}_pct"), 0) for m in GOALIE_METRICS},
                ...
                "g_added": 0.0,
                "g_prevented": _f(r.gsax_saved, 1),
                "g_net": _f(r.gsax_saved, 1),
                "gnet_pg": _f(r.gsax_saved / r.gp if r.gp else None, 3),
                "gprev60": _f(r.gprev60, 3),
            }
        )
        ...
        def _split(prefix):
            sog, ga, xga = (
                getattr(r, f"{prefix}_sog"),
                getattr(r, f"{prefix}_ga"),
                getattr(r, f"{prefix}_xga"),
            )
            return {
                "sa": int(sog),
        ...
        for s in seasons:
            ...
            rec = {
                ...
                "team": br.team,
                "gp": int(br.gp),
                "starts": int(br.starts),
                "toi_min": round(toi_s / 60.0, 1),
                "sa": sog,
                "sog_against": sog,
                "saves": int(br.saves),
                "ga": ga,
                "shutouts": int(br.shutouts),
                "sv_pct": round((sog - ga) / sog, 4) if sog else None,
                "gaa": round(ga * 3600.0 / toi_s, 2) if toi_s else None,
                "xga": round(float(br.xga), 1),
                "gsax": round(float(br.xga - ga), 1),
        ...
        detail = {
            ...
            "situation": situation,
            "value": {
                "rates": {"gprev60": {"v": _f(r.gprev60, 3), "pct": _f(r.gsax60_pct, 0)}},
                "actual": {
                    "added": 0.0,
                    "prevented": _f(r.gsax_saved, 1),
                    "net": _f(r.gsax_saved, 1),
                    "net_pg": _f(r.gsax_saved / r.gp if r.gp else None, 3),
                },
            },
            "shot_types": st.get(pid, []),
