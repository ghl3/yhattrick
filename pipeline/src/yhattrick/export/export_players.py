...
ELIGIBILITY = {
    "ev_off": ("ev_off_toi", MIN_EV_TOI),
    "ev_def": ("ev_def_toi", MIN_EV_TOI),
    "pp_off": ("pp_off_toi", 40),
    "pk_def": ("pk_def_toi", 40),
}
...
VALUE = {
    ...
    "playmaking60": (
        ...
        "ev_off_toi",
        MIN_EV_TOI,
    ),  # 5v5 creation for teammates/60 (on-ice − own ixG)
...
def full_seasons() -> list[int]:
    ...
    for s in model.available_seasons():
        n = pd.read_parquet(
            C.PROCESSED / "stints" / f"{s}.parquet", columns=["nhl_game_id"]
        ).nhl_game_id.nunique()
        if n >= FULL_MIN_GAMES:
            out.append(s)
    return out
...
def individual_table(fin: pd.DataFrame, career: pd.DataFrame) -> pd.DataFrame:
    ...
    df = career.merge(
        fin[["player_id", "shots", "ixg", "fin_per100", "fin_per100_se", "fin_goals"]],
        on="player_id",
        how="left",
    )
    sec = df.toi_all * 60.0
...
def shots_by_strength(seasons) -> pd.DataFrame:
    ...
    out = pd.DataFrame(
        {
            "player_id": df.shooter_id.astype(int).values,
            "shots5": is5,
            "shotspp": ispp,
            "ixg5": is5 * df.xg.values,
            "ixgpp": ispp * df.xg.values,
        }
    )
...
def penalty_value(seasons, allbox: pd.DataFrame) -> float:
    ...
    print(
        f"[value] penalty value V = {v:.4f} goals/penalty  (PP GF {pp_gf} − SH GF {sh_gf}) / {n_pen:.0f} drawn"
    )
    return v
...
def value_table(pooled: pd.DataFrame, shots_strength: pd.DataFrame, pen_v: float) -> pd.DataFrame:
    ...
    create60 = (
        df.ev_off_base / 5.0 + df.ev_off
    )  # 5v5 xG he creates (his share); incl. his own shots
    allow60 = (
        df.ev_off_base / 5.0 + df.ev_def
    )  # 5v5 opponent xG he allows (his share); lower better
    ...
    out["pen_net60"] = (
        df.pen_drawn60 - df.pen_taken60
    ) * pen_v  # net penalty goals / 60 (all situations)
...
def season_value(
    season: int, names: dict, pen_v: float, fin_s: pd.DataFrame, box_s: pd.DataFrame
) -> pd.DataFrame:
    ...
    ot = onice_table(box_s)[
        ["player_id", "ev_xgf60", "pp_xgf60"]
    ]  # on-ice xGF for the φ scoring/playmaking split
    car = career_totals(box_s)[["player_id", "toi_all", "c_pd", "c_pt"]]
    gp_s = box_s.groupby("player_id", as_index=False).gp.sum()
    ps = (
        ps.merge(ot, on="player_id", how="left")
        .merge(car, on="player_id", how="left")
        .merge(gp_s, on="player_id", how="left")
    )
    ...
    if fin_s is not None and len(fin_s):
        fs = fin_s.copy()
        fs["fin_goals"] = (fs.fin_per100.fillna(0.0) / 100.0) * fs.shots.fillna(
            0.0
        )  # season_finishing omits fin_goals
        ...
        out[int(r.player_id)].append(
            {
                ...
                "opp": r.opp,
                "home": bool(r.home),
                "gf": None if pd.isna(r.gf) else int(r.gf),
                "ga": None if pd.isna(r.ga) else int(r.ga),
...
def linemates(seasons, names, top=N_LINEMATES) -> dict[int, list]:
    ...
    for p, lst in mates.items():
        ...
        out[p] = [
            {"id": q, "name": names.get(q, {}).get("name", f"#{q}"), "toi_min": round(d / 60.0, 1)}
            for q, d in lst[:top]
        ]
    return out
...
def main() -> None:
    ...
    pooled = pooled.merge(
        individual_table(fin_df, career_totals(allbox)), on="player_id", how="left"
    )
    ...
    gp_df = (
        allbox.groupby("player_id", as_index=False).gp.sum()
        if len(allbox)
    ...
    pooled = pooled.merge(
        value_table(pooled, shots_by_strength(seasons), pen_v), on="player_id", how="left"
    )
    ...
    if gcp.exists():
        ...
        print(
            "[export] no gen_cards.json — details ship without the gen block "
    ...
    def valval(r, c):  # round a value metric: actual-goal totals to 1 dp, rates/per-game to 3 dp
        return _rnd(
            getattr(r, c), 1 if c.startswith("g_") else 3
    ...
    for r in table.itertuples():
        ...
        if gnp:
            ...
            for key, col in (
                ...
                ("pk_ga60", "pk_ga60"),
                ("scoring", "gen_scoring"),
                ("shooting", "gen_shooting"),
                ("finishing", "gen_finishing"),
                ("playmaking", "gen_playmaking"),
                ("defense", "gen_defense"),
            ):
                a = attrs.get(key)
                if a is not None:
        ...
        for s in seasons:
            ...
            rec = {
                "season": s,
                "team": brow.team,
                **{c: int(brow[c]) if c not in ("toi_min",) else float(brow[c]) for c in BOX_COLS},
            }
            si = simp.get(s)
            if si is not None and len(si):
                ...
                if len(ir):
                    rec.update(
                        {
                            m: (None if pd.isna(ir.iloc[0][m]) else round(float(ir.iloc[0][m]), 3))
                            for m in METRICS
                        }
                    )
            sv = sval.get(s)
            if sv is not None and len(sv):
                ...
                if len(vr):
                    rec.update(
                        {
                            c: (None if pd.isna(vr.iloc[0][c]) else round(float(vr.iloc[0][c]), 3))
                            for c in SEASON_VALUE_COLS
                        }
                    )
            fs = fin_season.get(s)
            if fs is not None and len(fs):
                ...
                if len(fr) and sec > 0:
                    ...
                    rec["xg_per_shot"] = (
                        round(float(fr.ixg) / fr.shots, 4) if fr.shots > 0 else None
                    )
                    rec["fin_per100"] = (
                        None if pd.isna(fr.fin_per100) else round(float(fr.fin_per100), 2)
                    )
        ...
        detail = {
            ...
            "impact": {
                m: {
                    "v": getattr(r, m),
                    "se": getattr(r, f"{m}_se"),
                    "toi": getattr(r, f"{m}_toi"),
                    "pct": getattr(r, f"{m}_pct"),
                }
                for m in METRICS
            },
            ...
            "individual": {
                c: {
                    "v": indval(r, c),
                    "pct": getattr(r, f"{c}_pct"),
                    **({"se": _rnd(getattr(r, "fin_per100_se"), 2)} if c == "fin_per100" else {}),
                }
                for c in INDIVIDUAL
            },
            "shooting": {
                "shots": int(_rnd(getattr(r, "shots"), 0) or 0),
            ...
            "value": {
                "rates": {
                    ...
                    "playmaking60": {
                        "v": valval(r, "playmaking60"),
                        "pct": getattr(r, "playmaking60_pct"),
                    },
                    "allow60": {"v": valval(r, "allow60"), "pct": getattr(r, "allow60_pct")},
                    "pp_scoring60": {
                        "v": valval(r, "pp_scoring60"),
                        "pct": getattr(r, "pp_scoring60_pct"),
                    },
                    "pp_playmaking60": {
                        "v": valval(r, "pp_playmaking60"),
                        "pct": getattr(r, "pp_playmaking60_pct"),
                    },
                    "pk_allow60": {
                        "v": valval(r, "pk_allow60"),
                        "pct": getattr(r, "pk_allow60_pct"),
                    },
                    "pen_net60": {"v": valval(r, "pen_net60"), "pct": getattr(r, "pen_net60_pct")},
                },
                "actual": {
                    "create_pg": valval(r, "gcreate_pg"),
                    "allow_pg": valval(r, "gallow_pg"),
                    "net_pg": valval(r, "gnet_pg"),
                    "net_pg_pct": getattr(r, "gnet_pg_pct"),
                    "created": valval(r, "g_created"),
                    "allowed": valval(r, "g_allowed"),
                    "fin": valval(r, "g_fin"),
                    "pen": valval(r, "g_pen"),
                    "net": valval(r, "g_net"),
                },
