...
_KEEP_EVENTS = {
    "faceoff", "shot-on-goal", "missed-shot", "blocked-shot", "goal",
    "penalty", "hit", "giveaway", "takeaway", "stoppage",
}
...
def _roster_lookup(season: int) -> dict[int, dict]:
    ...
    return {int(row.player_id): {"id": int(row.player_id), "name": row.player_name,
                                 "pos": row.position, "number": row.number}
...
def _game_meta(gid: int) -> dict:
    ...
    return {
        ...
        "home_score": pbp["homeTeam"].get("score"), "away_score": pbp["awayTeam"].get("score"),
    }
...
def _game_aggregates(gid, stints, shifts_g, shots_g, events_g, meta, lookup) -> tuple[dict, list]:
    ...
    totals = {
        ...
        "home_shots": int((shots_g.is_home == 1).sum()), "away_shots": int((shots_g.is_home == 0).sum()),
    ...
    for pid, row in sh.iterrows():
        ...
        players.append({
            "id": pid, "name": info["name"], "pos": info["pos"], "number": info["number"],
            "team": row.team, "side": "home" if row.team == home else "away",
            "shifts": int(row.shifts), "toi_s": int(row.toi_s),
            "g": g, "a1": p1, "a2": p2, "pts": g + p1 + p2,
