...
from ..models.player_onice_model import (
    MIN_STINT_S,
)  # same stint filter the model uses, for a fair cross-check
        ...
        if s.strength == "5v5":
            ...
            if s.home_n > s.away_n:  # home on the power play
                for pid in s.home_skaters:
                    a = acc[int(pid)]
                    a[PPX] += hx
                    a[PPT] += dur  # PP offence
                for pid in s.away_skaters:
                    a = acc[int(pid)]
                    a[PKX] += hx
                    a[PKT] += dur  # PK: xG allowed
...
def ice_and_team(sh: pd.DataFrame) -> pd.DataFrame:
    ...
    by_team = (
        sh.groupby(["player_id", "team"])
        .nhl_game_id.nunique()
        .reset_index()
        .sort_values("nhl_game_id", ascending=False)
    )
    primary = by_team.groupby("player_id").team.first()
    teams = by_team.groupby("player_id").team.apply(list)
    return pd.DataFrame(
        {"gp": g.gp, "toi_s": g.toi_s, "team": primary, "teams": teams}
    ).reset_index()
...
def _box_for(gids, sh, roster, season, game_type) -> pd.DataFrame:
    ...
    box = (
        ice_and_team(sh)
        .merge(counts, on="player_id", how="left")
        .merge(roster, on="player_id", how="left")
    )
...
def build_season(season: int) -> int:
    ...
    roster = pd.read_parquet(C.INTERIM / "roster" / f"{season}.parquet")[
    ...
    for gt, sub in box.groupby("game_type"):
        print(
            f"[box] {C.season_label(season)} {gt}: {len(sub)} players, {sub.gp.sum()} player-games"
        )
    return len(box)
