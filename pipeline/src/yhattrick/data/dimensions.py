...
def build_players() -> pd.DataFrame:
    ...
    df = pd.DataFrame(rows, columns=["player_id", "name", "birthdate", "shoots", "height_in",
                                     "weight_lb", "draft_year", "draft_overall"])
    ...
    for s in sorted(set(int(x) for x in seasons)):
        for pf in sorted(C.RAW_PBP.glob(f"{s}0*.json")):
            ...
            rows.append({
                ...
                "raw_venue": _default(d.get("venue")) or (d.get("homeTeam") or {}).get("abbrev"),
            })
    return pd.DataFrame(rows, columns=["nhl_game_id", "date", "season", "game_type",
                                       "home_team", "away_team", "raw_venue"])
...
def build_arenas(games_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    for canon, sub in g.groupby("canonical"):
        rows.append({"canonical_name": canon,
...
def build_games(games_raw: pd.DataFrame, arenas: pd.DataFrame) -> pd.DataFrame:
    ...
    return (g[["nhl_game_id", "date", "season", "game_type", "home_team", "away_team", "venue_id"]]
            .sort_values("nhl_game_id").reset_index(drop=True))
...
def main() -> None:
    ...
    print(f"[dimensions] players: {len(players):,} rows "
    ...
    print(f"[dimensions] player_season: {len(ps):,} rows "
    ...
    print(f"[dimensions] arenas: {len(arenas):,} venues "
    ...
    print(f"[dimensions] games: {len(games):,} rows ({reg:,} regular-season, "
