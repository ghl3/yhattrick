...
def test_conversion_nll_equals_bce_plus_ridge():
    ...
    si = np.array([0, 0, 0]); sh = np.array([0, 1, 0]); go = np.array([1, 0, 1])
    lxg = np.array([-1.0, 0.5, 0.2]); Cx = np.zeros((3, 0)); y = np.array([0.0, 1.0, 0.0])

    a, b, fin, gsave = th[0], th[1], th[2:4], th[4:6]
    eta = a * lxg + b + fin[sh] + gsave[go]
    expected = np.sum(np.logaddexp(0.0, eta) - y * eta) + 0.5 * (lf * np.sum(fin ** 2) + lg * np.sum(gsave ** 2))
