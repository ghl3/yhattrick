...
def test_event_counts_tallies_and_points(tmp_path, monkeypatch):
    ...
    p10 = box.loc[10]
    assert (
        p10.g == 1 and p10.sog == 2 and p10.icf == 3
    )  # goal: g+sog+icf; sog: sog+icf; missed: icf
    assert p10.fo_won == 1
    ...
    assert (
        box.loc[13].icf == 1 and box.loc[13].takeaways == 1
    )  # blocked-shot shooter gets a Corsi-for
