...
def test_report_counts_exact_failures():
    cs = [core.exact("a", "s", True), core.exact("b", "s", False),
          core.approx("c", "s", 9.0, None, 1.0), core.skip("d", "s", "EXACT", "n/a")]
    res = core.report(cs)
...
def _clean_stints():
    rows = [
        dict(nhl_game_id=1, stint_idx=0, start_g=0, end_g=100, duration_s=100,
             home_skaters=[1, 2, 3, 4, 5], away_skaters=[6, 7, 8, 9, 10], home_n=5, away_n=5,
             strength="5v5", home_xgf=0.6, away_xgf=0.0, overload=False),
        dict(nhl_game_id=1, stint_idx=1, start_g=100, end_g=200, duration_s=100,
             home_skaters=[1, 2, 3, 4, 5], away_skaters=[6, 7, 8, 9, 10], home_n=5, away_n=5,
             strength="5v5", home_xgf=0.4, away_xgf=0.2, overload=False),
    ]
    shots = [
        dict(nhl_game_id=1, stint_idx=0, xg=0.6, goal=1, onice_match="exact", event="goal",
             strength="5v5", sit_home_n=5, sit_away_n=5),
        dict(nhl_game_id=1, stint_idx=1, xg=0.4, goal=0, onice_match="exact", event="shot-on-goal",
             strength="5v5", sit_home_n=5, sit_away_n=5),
        dict(nhl_game_id=1, stint_idx=1, xg=0.2, goal=0, onice_match="within1", event="missed-shot",
             strength="5v5", sit_home_n=5, sit_away_n=5),
    ]
    return rows, shots
...
def _player(**over):
    base = dict(id=1, g_created=8.0, g_fin=1.5, g_allowed=3.0, g_pen=0.0, g_net=6.5,
                scoring60=0.14, playmaking60=0.76, allow60=0.3, gnet_pg_pct=90.0)
    base.update(over)
    return base
...
def _ev_row(**over):
    base = dict(player_id=1, ev_off=0.0, ev_def=0.0, ev_off_base=2.5,
                ev_off_toi=600.0, ev_def_toi=600.0)   # toi in minutes
    base.update(over)
    return base


def _xgf_stint(hxgf, axgf, **over):
    base = dict(nhl_game_id=2024020001, strength="5v5", overload=False, duration_s=100,
                home_xgf=hxgf, away_xgf=axgf)
    base.update(over)
    return base
...
def test_ev_creation_reconciles_when_consistent(tmp_path, monkeypatch):
    ...
    _write_ev_and_stints(tmp_path / "models", tmp_path / "proc",
                         [_ev_row()], [_xgf_stint(3.0, 2.0)])
...
def test_ev_creation_warns_when_toi_accounting_breaks(tmp_path, monkeypatch):
    ...
    _write_ev_and_stints(tmp_path / "models", tmp_path / "proc",
                         [_ev_row(ev_off_toi=6000.0)], [_xgf_stint(3.0, 2.0)])
    out = list(checks._ev_creation_reconciliation([2024], "2024", "models"))
