...
from yhattrick.data.stints import (
    assign_other,
    assign_shotlike,
    build_stints_for_game,
    classify_onice_match,
    drop_foreign_shifts,
    other_stint_index,
    personnel_per_stint,
    shot_stint_index,
...
def _pp_goal_game():
    ...
    shots = pd.DataFrame(
        {"time_g": [100], "is_home": [1], "xg": [0.6]}
    )  # HOME goal at the boundary
    events = pd.DataFrame(
        {
            "time_g": [100, 100],
            "type": ["goal", "faceoff"],
    ...
    events = pd.DataFrame(
        {
            ...
            "is_home": [
                ...
                True,
                False,
            ],  # blocked-shot is owned by the blocking team
...
def test_assign_shotlike_upper_and_lower_guards():
    ...
    assert list(idx) == [
        ...
        -1,
        -1,
    ]  # 250 past end -> drop; 200 ends stint1; 0/-5 pre-first
...
def test_personnel_boundary_off_by_one():
    ...
    rows = [
        (0, 200, 1, "AAA"),
        (0, 200, 2, "BBB"),  # goalies, full game
        (0, 100, 10, "AAA"),
        (100, 200, 11, "AAA"),  # A then B
        (0, 200, 20, "BBB"),
    ]
...
def test_personnel_strengths_3v3_and_pulled_goalie():
    ...
    pulled = (
        [(0, 100, 2, "BBB")]  # only AWAY has a goalie
        + [(0, 100, p, "AAA") for p in range(10, 16)]  # HOME pulled: 6 skaters
        + [(0, 100, p, "BBB") for p in range(20, 25)]
    )  # AWAY: 5 skaters
    ...
    events = pd.DataFrame(
        {
            ...
            "is_home": [True, True],
            "zone": [None, None],
            "period": [1, 5],  # the 2nd is a shootout decider
        }
    )
    st = _by_strength(build_stints_for_game(shifts, shots, events, goalies, home))
...
def test_real_goals_strength_matches_situationcode(season):
    ...
    assert (
        mismatch < 0.03
    ), f"{season}: {mismatch:.1%} of goals misfiled by strength (boundary bug?)"
