...
def _synth_create(P=70, n=120000, dur=45.0, seed=4, r_disp=None, arena_fx=None):
    ...
    R = {
        ...
        "idx": {p: p for p in range(P)},
        "shooter_idx": shooter,
        "team_idx": team,
        "def_idx": dff,
        "Xctx": Xctx,
        "count": count,
        "offset": np.full(n, np.log(dur / 3600.0)),
        "dur": np.full(n, dur),
    ...
    pr = np.exp(np.concatenate([np.full((ng, 1), create_0), create[gt]], 1))
    pr /= pr.sum(1, keepdims=True)
    gc = np.array([rng.choice(5, p=pr[i]) for i in range(ng)]).astype(
        np.int64
    )  # 0=unassist, 1..4 col
...
def _synth_creator(P=60, n=150000, seed=2, qcF=0.30, qcD=-0.20):
    ...
    creator = np.where(
        goal == 1, np.where(c == 0, 4, c - 1), -1
    )  # goals observe creator; else latent
    Q = {
        "shooter_idx": shooter,
        "team_idx": team,
        "def_idx": dff,
        ...
        "y": y,
        "goal": goal,
        "creator": creator.astype(np.int64),
    }
...
def _synth_conversion(P=60, Gg=20, n=200000, seed=5):
    ...
    fin = rng.normal(0, 0.20, P)
    fin -= fin.mean()
    gsave = rng.normal(0, 0.20, Gg)
    gsave -= gsave.mean()
    ...
    Cr = {
        "shooter_idx": shooter.astype(np.int64),
        "goalie_idx": goalie.astype(np.int64),
        "logit_xg": lxg,
        "y": y,
        "goalies": list(range(Gg)),
    }
...
def test_rate_create_recovers_shoot_create_def():
    ...
    assert (
        np.corrcoef(fit["create"], create)[0, 1] > 0.8
    )  # teammate-lift + assist credit (playmaking)
...
def test_rate_create_reconciles_total_shots():
    ...
    eta = (
        fit["intercept"]
        + fit["shoot"][R["shooter_idx"]]
        + fit["create"][R["team_idx"]].sum(1)
        + fit["def"][R["def_idx"]].sum(1)
        + R["Xctx"] @ fit["beta"]
    )
...
def test_shoot_and_create_are_separately_identified():
    ...
    fit = G.fit_rate_create(R, gt, gc, shots_per_goal=16)
    assert (
        np.corrcoef(fit["shoot"], shoot)[0, 1] > abs(np.corrcoef(fit["shoot"], create)[0, 1]) + 0.3
    )
...
def test_player_values_merge_signs():
    ...
    mq = np.log(0.07 / 0.93)
    rate = {
        "intercept": np.log(8.0),
        "shoot": np.array([0.4, 0.0, 0.0]),
        "create": np.array([0.0, 0.5, 0.0]),
        "def": np.array([0.0, 0.0, -0.4]),
        "psi0": -0.5,
    }
    qual = {
        "mu_qual": {"ev": mq},
        "qshoot": np.zeros(P),
        "qcreate": np.array([0.0, 0.5, 0.0]),
        "qdef": np.zeros(P),
    }
    ...
    assert v["playmaking"][1] == max(
        v["playmaking"]
    )  # player 1 (high create + qcreate) tops playmaking
...
def _synth_states(
    P=40, S=3, n_per=60000, dur=45.0, seed=9, drift_player=0, drift=0.35, age_cols=False
):
    ...
    lam = (
        np.exp(
            ...
            + w1 * zsh
            + w2 * zsh**2
            + create[team, srow[:, None]].sum(1)
            + deff[dff_, srow[:, None]].sum(1)
        )
        * dur
        / 3600.0
    )
    ...
    R = {
        ...
        "def_goalie": np.zeros(n, dtype=object),
        "toi": np.full(P, 1e6),
        "season_row": srow + 2021,
        "seasons": seasons,
        "n_season_cols": 0,
        "last_season": np.full(P, seasons[-1], dtype=np.int64),
        "unit_player": up,
        "unit_season": us + 2021,
        "unit_lut": np.arange(P * S).reshape(P, S),
        "n_units": P * S,
        ...
        "e_prev": e_prev,
        "e_next": e_prev + 1,
        "e_gap": np.ones(len(e_prev)),
        "first_mask": (us == 0).astype(float),
        "toi_unit": np.full(P * S, 1e6),
    }
    ng = 3000 * S
...
def test_age_curve_recovery_and_projection():
    ...
    rp, _, _ = G.effective_params(
        {"ev": fit}, qual, conv, R["players"], agepos, last_season, target=seasons[-1] + 1
    )
...
def test_sparse_dense_se_parity():
    ...
    fit_sparse = G.fit_rate_create(
        R, gt, gc, shots_per_goal=16, dense_max=1
    )  # forced sparse splu solve
...
def test_conversion_season_offsets_reconcile():
    ...
    Cr = {
        ...
        "goalies": list(range(Gg)),
        "season": season,
        "ctx": (season == 2022).astype(float)[:, None],
        "ctx_names": ["season_2022"],
    }
...
def test_rate_warm_start_maps_by_key_not_position():
    ...
    w2 = dict(
        w,
        theta=th,
        unit_pid=np.asarray(w["unit_pid"])[perm],
...
def test_rate_warm_start_carries_arena_states():
    ...
    assert (
        warm["converged"] and warm["nit"] < cold["nit"]
    )  # carried the states → started at the optimum
...
def test_quality_and_conversion_warm_reuse():
    ...
    q3 = G.fit_quality_creator(
        Q, P=30, creates={0: (create, -0.5)}, isD=isD, warm=wq, reuse=True, pids=pids
    )
...
def test_ckpt_save_load_roundtrip(tmp_path):
    ...
    q2 = G.fit_quality_creator(
        Q,
        P=25,
        creates={0: (create, -0.5)},
        isD=isD,
        warm=G._ck_stage(ck, "qual"),
        reuse=True,
        pids=R["players"],
    )
...
def _synth_fixed_units(P=10, n=40000, seed=11, role_player=0):
    ...
    atk = np.array(
        [np.concatenate([units[u][k:], units[u][:k]]) for u, k in zip(rowsu, rng.integers(0, 5, n))]
    )
    ...
    for i in range(ng):  # shooter ≠ role player; A1 = role player
        u = units[gu[i]]
        sh_i = (
            u[(1 + rng.integers(0, 4))]
            if u[0] == role_player * (1 - gu[i])
            else u[rng.integers(0, 5)]
        )
